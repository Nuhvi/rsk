...
impl Ecies {
    ...
    pub fn encrypt(&self, data_in: Bytes, data_out: &mut BytesMut) -> Result<usize> {
        ...
        let mut data_buf = BytesMut::from(data_in.as_ref());
        let iv_clone = iv;
        let mut encryptor = Aes128Ctr64BE::new(
            encryption_key.as_ref().into(),
            iv_clone.as_ref().into(),
        );
        encryptor.apply_keystream(&mut data_buf);

        let tag = self.calculate_tag(&mac_key, &iv, &total_size.to_be_bytes(), &data_buf)?;
        ...
        data_out.extend_from_slice(&data_buf);
